...
import static perf_statistic.common.PluginConstants.TAG_PERFORMANCE_STATISTICS;
...
public class BuildHistoryRefCheckAdapter extends BuildServerAdapter {
	...
	private static final String CHECK_REFERENCE_ACTIVITY_NAME_TAGS = "Check reference values calculated from tagged builds";
...
//		    calculate ref values form tagged builds
			Map<String, List<BigDecimal>> tagValues = new HashMap<String, List<BigDecimal>>();
			SBuildType buildType = runningBuild.getBuildType();
			if (buildType != null) {
				for (String key : currentValues.keySet()) {
					if (currentValues.get(key) == null)
						continue;
//					init array
					List<BigDecimal> values = tagValues.get(key);
					if (values == null) {
						values = new ArrayList<BigDecimal>();
					}
//					extract all values from tagged builds

					List<SFinishedBuild> taggedBuilds = new ArrayList<SFinishedBuild>();
					for (SFinishedBuild build : buildType.getHistory()) {
						if (build.getTags().contains(TAG_PERFORMANCE_STATISTICS)) {
							taggedBuilds.add(build);
						}
					}

					List<BigDecimal> finalValues = values;

					for (SFinishedBuild build : taggedBuilds) {
						BigDecimal val = myStorage.getValue(key, build.getBuildId());
						if (val != null) {
							finalValues.add(val);
						}
					}
					tagValues.put(key, finalValues);
				}
	...
	private void processValues(Map<String, List<BigDecimal>> relatedValues, Map<String, PerformanceMessage> currentValues, SRunningBuild runningBuild, ParametersProvider parametersProvider) {
		...
		if (!relatedValues.isEmpty()) {
			for (String key : currentValues.keySet()) {
				...
				List<BigDecimal> values = relatedValues.get(key);
